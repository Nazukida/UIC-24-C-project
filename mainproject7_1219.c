...
void room_output(int room_size, int total_room, char *time, int day) {
    ...
    for (int i = 0; i < total_room; i++) {
        ...
        fp = fopen(filename, "r");
        if (fgets(line, sizeof(line), fp) != NULL) {//逐行读取
            line[strcspn(line, "\n")] = '\0';
            int file_room_size = atoi(line);
            if (file_room_size < room_size) {//确定大小
                fclose(fp);
                continue;
            }
        }
        int today_flag = 0, found_flag = 0;
        while (fgets(line, sizeof(line), fp) != NULL) {//检索对应模块
            line[strcspn(line, "\n")] = '\0';
            if (strcmp(line, "Today") == 0) {//进行检查
                today_flag = 1;
                continue;
...
void book(int class, char *time_flag, int day) {//直接替换新文件hhh
    char filename[20];
    char filename_ss[20];
