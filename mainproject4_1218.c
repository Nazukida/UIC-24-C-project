...
void book(int class, char *time_flag, int day) {
    ...
    char file_line[100];//一行内容暂时存放
    int flag_day = 0;
    while (fgets(file_line, sizeof(file_line), fp)!= NULL) {
        file_line[strcspn(file_line, "\n")] = '\0';//查指针
        if ((day == 1 && strcmp(file_line, "Today") == 0) || (day == 2 && strcmp(file_line, "Tomorrow") == 0) ||(day == 3 && strcmp(file_line, "The day after Tomorrow") == 0)) {//只要有就把标签设置为1
            flag_day = 1;
            continue;
        }
        if (flag_day) {
            if (strstr(file_line, time_flag)!= NULL/*判断时间*/ && strstr(file_line, "|: yes")!= NULL/*判断是否可定*/) {
                char *flag = strstr(file_line, "|: yes");//直接查指针，直接定位然后修改
                strcpy(flag, "|: no ");
                char delete[30] = "                           ";
                fseek(fp, -strlen(file_line)-3, SEEK_CUR);//回退光标
                fputs(delete, fp);
